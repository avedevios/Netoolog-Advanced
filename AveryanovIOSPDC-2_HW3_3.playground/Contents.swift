...
let dealershipBMWFirst  = DealerBMW(name: "BMW dealership #1", showroomCapacity: 3)
...
protocol SpecialOffer {
    ...
    func discountCalculation(_ buildDateYear: Int) throws -> Int
    func moveToShowroom(_ car: inout Carable) throws
}
...
extension DealerMain: SpecialOffer {
    ...
    func addEmergencyPack() {
        ...
        for index in 0...cars.count - 1 {
            if cars[index].salePrecentage != nil {
    ...
    func makeSpecialOffer() {
        ...
        for index in 0...cars.count - 1 {
            if let buildDateYear = cars[index].buildDate[DateSections.year] {
                cars[index].salePrecentage = try? discountCalculation(buildDateYear)
                
                if cars[index].salePrecentage == nil {
                    print("Error! \(cars[index].model) with id: \(cars[index].id) is younger then 2021")
                } else {
        ...
        for index in 0...cars.count - 1 {
            if cars[index].salePrecentage != nil {
                do {
                    try moveToShowroom(&cars[index])
                }
                catch {
                    print("Error! \(cars[index].model) with id: \(cars[index].id) already in showroom")
                }
            }
        }
        addEmergencyPack()
    ...
    func discountCalculation(_ buildDateYear: Int) throws -> Int {
        guard buildDateYear < 2022 else {
            throw Errors.carYounger2021
        }
    ...
    func moveToShowroom(_ car: inout Carable) throws {
        guard !showRoom.contains(where: {$0.id == car.id})
        else {
            throw Errors.carAlreadyInShowroom
        }
